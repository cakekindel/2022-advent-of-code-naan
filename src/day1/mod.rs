...
//! they've brought with them, one item per line.
...
impl Elf {
  pub fn parse<S: AsRef<str>>(s: S) -> Vec<Elf> {
    s.as_ref()
     .trim()
     .split("\n\n")
     .map(|nums| {
     ...
     .map(|snacks| Elf { snacks })
     .collect()
  }

  pub fn total(&self) -> u32 {
    self.snacks.foldl_ref(|sum, n| sum + n, 0)
  }
}
...
pub fn most_caloric_elf<S: AsRef<str>>(input: S) -> u32 {
  let elves = Elf::parse(input);

  elves.foldl(|biggest: Option<Elf>, e: Elf| {
                Some(biggest.filter(|b| b.total() > e.total()).unwrap_or(e))
              },
              None)
       .unwrap()
       .total()
...
pub fn total_calories_of_top_three<S: AsRef<str>>(input: S) -> u32 {
  let elves = Elf::parse(input);
  elves.foldl(|biggest_3: Vec<Elf>, e: Elf| {
                if biggest_3.len() < 3 {
                  biggest_3.append(vec![e.clone()])
                } else if biggest_3.any(|b: &Elf| b.total() < e.total()) {
                  let smallest =
                    biggest_3.clone().foldl(|smallest: Option<Elf>, b: Elf| {
                                              Some(smallest.filter(|s| s.total() < b.total())
                                                           .unwrap_or(b))
                                            },
                                            None);
                  biggest_3.into_iter()
                           .filter(|e| Some(e) != smallest.as_ref())
                           .collect::<Vec<_>>()
                           .append(vec![e])
                } else {
                  biggest_3
              ...
              vec![])
       .foldl(|sum: u32, n: Elf| sum + n.total(), 0)
}
