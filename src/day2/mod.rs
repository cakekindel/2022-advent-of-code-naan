...
impl Move {
  ...
  pub fn outcome(&self, other: &Self) -> Outcome {
    ...
    match (self, other) {
      | (Paper, Rock) | (Rock, Scissors) | (Scissors, Paper) => Outcome::Win,
...
impl Round {
  pub fn score(&self) -> u32 {
    self.us.score() + self.us.outcome(&self.them).score()
  }
}
...
impl Strategy {
  pub fn parse_interpret_column2_as_move<S: AsRef<str>>(s: S) -> Self {
    Self(s.as_ref().trim().split("\n").map(|row| row.trim().split(' ').collect::<Vec<_>>()).map(|moves| Round {them: Move::parse(moves[0]), us: Move::parse(moves[1])}).collect::<Vec<_>>())
  }

  pub fn parse_interpret_column2_as_outcome<S: AsRef<str>>(s: S) -> Self {
    Self(s.as_ref().trim().split("\n").map(|row| row.trim().split(' ').collect::<Vec<_>>()).map(|moves| Round {them: Move::parse(moves[0]), us: Move::parse(moves[0]).move_for_outcome(Outcome::parse(moves[1]))}).collect::<Vec<_>>())
  }

  pub fn score(&self) -> u32 {
    self.0.clone().foldl(|s: u32, rd: Round| s + rd.score(), 0)
  }
}
...
mod test {
  ...
  fn part_1_sample_input() {
    assert_eq!(Strategy::parse_interpret_column2_as_move(include_str!("./input.sample")).score(), 15)
  }

  #[test]
  fn part_1() {
    assert_eq!(Strategy::parse_interpret_column2_as_move(include_str!("./input")).score(), 13_682)
  }

  #[test]
  fn part_2_sample_input() {
    assert_eq!(Strategy::parse_interpret_column2_as_outcome(include_str!("./input.sample")).score(), 12)
  }

  #[test]
  fn part_2() {
    assert_eq!(Strategy::parse_interpret_column2_as_outcome(include_str!("./input")).score(), 12_881)
  }
}
