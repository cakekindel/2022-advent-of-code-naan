...
impl Strategy {
  pub fn parse_interpret_column2_as_move<S: AsRef<str>>(s: S) -> Self {
    Self(s.as_ref()
          ...
          .map(|moves| Round { them: Move::parse(moves[0]),
                               us: Move::parse(moves[1]) })
          .collect::<Vec<_>>())
  }
  ...
  pub fn parse_interpret_column2_as_outcome<S: AsRef<str>>(s: S) -> Self {
    Self(s.as_ref()
          ...
          .map(|row| row.trim().split(' ').collect::<Vec<_>>())
          .map(|moves| Round { them: Move::parse(moves[0]),
                               us: Move::parse(moves[0]).move_for_outcome(Outcome::parse(moves
                                                                                           [1])) })
          .collect::<Vec<_>>())
  }
...
mod test {
  ...
  fn part_1_sample_input() {
    assert_eq!(Strategy::parse_interpret_column2_as_move(include_str!("./input.sample")).score(),
               15)
  }

  #[test]
  fn part_1() {
    assert_eq!(Strategy::parse_interpret_column2_as_move(include_str!("./input")).score(),
               13_682)
  }

  #[test]
  fn part_2_sample_input() {
    assert_eq!(Strategy::parse_interpret_column2_as_outcome(include_str!("./input.sample")).score(),
               12)
  }

  #[test]
  fn part_2() {
    assert_eq!(Strategy::parse_interpret_column2_as_outcome(include_str!("./input")).score(),
               12_881)
  }
}
